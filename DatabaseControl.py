...
class DatabaseControl:
    ...
    def __init__(self,config):
        self.output = False
        self.config = config
        self.cnx = mysql.connector.connect(user=config["credenciales"]["user"],database=config["credenciales"]["database"],password=config["credenciales"]["password"])
        self.cursor = self.cnx.cursor()        

    def createTable(self):
        #self.tableName = tableName
        tableName = self.config["tableName"]
        sqlDrop = "DROP TABLE IF EXISTS `"+self.config["credenciales"]["database"]+"`.`"+tableName+"`;"
